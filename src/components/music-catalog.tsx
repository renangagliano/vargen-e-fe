...
export function MusicCatalog({ songs, knowledgeBaseSearchIndex = {} }: { songs: Song[]; knowledgeBaseSearchIndex?: Record<string, string> }) {
  ...
  const filtered = useMemo(() => songs.filter((song) => {
    ...
    return (!normalized || `${song.title} ${song.celebration} ${song.scripture.join(" ")} ${knowledgeBaseSearchIndex[song.slug] ?? ""}`.toLocaleLowerCase("pt-BR").includes(normalized)) && (season === "all" || song.liturgicalSeason === season) && (year === "all" || song.liturgicalYear === year) && (celebration === "all" || song.celebration === celebration);
  }), [songs, query, season, year, celebration, knowledgeBaseSearchIndex]);
