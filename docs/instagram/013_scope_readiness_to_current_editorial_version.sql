-- Section 11.7.5 follow-up: bind readiness recomputation to the current editorial version.
...
begin
  ...
  if p_action = 'save_editorial' then
    ...
    if v_reference is not null and (length(v_reference) > 120 or v_reference !~ '^([1-3][[:space:]]+)?[[:alpha:]]+([[:space:]]+[[:alpha:]]+)*[[:space:]]+[0-9]+(,[0-9]+(-[0-9]+)?(\.[0-9]+(-[0-9]+)?)*)?$') then
    ...
    return jsonb_build_object('action', p_action, 'reel_id', p_reel_id, 'editorial_version', v_next, 'idempotent', false, 'state', to_jsonb(v_editorial), 'bible_status', case when v_reference is null then 'MISSING' else 'VERIFIED' end);
  end if;
