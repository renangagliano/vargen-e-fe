-- Section 11.7: controlled remote governance writes.
-- Apply only after the SQLite/Supabase cutover checklist is approved. The
-- application currently remains fail-closed with ADMIN_REMOTE_WRITE_ENABLED=false.
-- This function is intentionally executable by service_role only; the Admin
-- server authenticates the operator and role before invoking it.
...
begin
  ...
  if p_action = 'verify_bible' then
    ...
    if not found or nullif(trim(v_reference), '') is null then raise exception 'BIBLE_REFERENCE_NOT_FOUND'; end if;
