# Estratégia de conteúdo — Fases 4 e 6

Esta é uma hipótese editorial para testar após o piloto; não é uma promessa de alcance ou monetização.

## Posicionamento

Vargen & Fé transforma Escritura e liturgia em música, combinando rock católico, folk/nórdico, classic rock, southern rock e linguagem cinematográfica cristã.

Mensagem central: **A Bíblia transformada em música.**

## Pilares

- `SCRIPTURE`
- `FAITH`
- `OVERCOMING`
- `GRATITUDE`
- `PRAYER`
- `LITURGICAL_YEAR`
- `DAILY_FAITH`
- `WEEKLY_JOURNEY`
- `MONTHLY_JOURNEY`
- `DEVOTIONAL`
- `HOLY_FAMILY`
- `BEHIND_THE_SONG`
- `FULL_RELEASE`
- `MUSICAL_CLIMAX`

O catálogo real descoberto contém categorias como `7 Dias com Deus  Fé, Força e Superação`, `Advento`, `Anunciação`, `Domingo da Páscoa`, `Quaresma`, `Solenidades`, `Tempo Comum`, `Tempo do Natal` e `12 Meses com Deus`. As entradas de `src/data/songs.ts` são a autoridade; não inventar músicas, referências ou datas.

Na Fase 6, 78 masters locais geraram 234 Reels. A Fase 6.1 recalibrou o
portfólio para 111 ativos e 123 em espera, preservando todos os arquivos para
revisão futura. O inventário técnico fica em Reels/catalog-review.html e a
fila curada em Reels/catalog-curation.html.

## Formatos a testar

- hook lírico curto;
- refrão com frase completa;
- passagem bíblica contextualizada;
- clímax instrumental;
- bastidor da criação;
- chamada para a versão completa no YouTube/site.

Não publicar variantes quase idênticas em sequência. Um mesmo song pode ter vários momentos, desde que tenham timestamps, hook e função editorial distintos.

## Regras editoriais

- qualidade antes de volume;
- perguntas autênticas, sem engagement bait;
- CTA rotativo e coerente;
- hashtags curtas e contextuais;
- não prometer milagres, dinheiro ou resultado espiritual;
- não comprar seguidores, likes, comentários ou visualizações;
- usar somente áudio/visual com direitos comprovados;
- manter `RIGHTS_PENDING_CONFIRMATION` como gate até confirmação deliberada;
- manter todo pacote em `READY_FOR_HUMAN_REVIEW` até decisão humana;
- marcar `BIBLE_REFERENCE_REVIEW_REQUIRED` quando a fonte não validar uma
  referência precisa;
- considerar calendário litúrgico somente com datas verificadas.

## Métrica de sucesso

Priorizar retenção, conclusão, compartilhamentos, salvamentos, visitas ao perfil, seguidores qualificados e cliques para YouTube/site. Visualizações isoladas não bastam.

## Hipóteses iniciais

Serão registradas como experimentos, não como fatos:

1. hooks bíblicos podem gerar mais salvamentos que hooks genéricos;
2. refrões completos podem superar cortes sem frase concluída em retenção;
3. composição vertical que preserve o sujeito pode superar crop agressivo;
4. conteúdo litúrgico pode ter melhor relevância em janelas próximas à celebração.

Cada hipótese precisa de controle, variante, métrica primária, janela e amostra antes de gerar recomendação.

## Pacotes produzidos na Fase 6

Os geradores catalogais produzem título, hook, caption, CTA, hashtags,
content pillar, cover text e espaçamento recomendado. A curadoria acrescenta
ranking relativo, valor incremental, tier e decisão de portfólio. Não produzem
promessas de milagre, dinheiro ou engajamento artificial. Os pacotes não são
publicáveis enquanto direitos, revisão humana, referência bíblica e gates
técnicos não forem confirmados.
