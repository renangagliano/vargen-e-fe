...
function bestForDuration(report: MediaAnalysisReport, durationMs: number, category: CandidateCategory, selected: ReelCandidate[]): ReelCandidate {
  ...
  for (let start = 0; start <= lastStart; start += 500) {
    ...
    const candidate: ReelCandidate = {
      candidateId: candidateId(report.sourceAssetId, start, end),
      ...
      status: "PROPOSED",
      fingerprint: candidateFingerprint(report.sourceAssetId, start, end),
    };
