...
function persist(db: ReturnType<typeof openDatabase>, result: InternalEvaluation, actor: string): void {
  ...
  db.prepare("INSERT OR IGNORE INTO publication_audit_events (event_id, entity_type, entity_id, event_type, actor, timestamp, metadata_json_safe) VALUES (?, ?, ?, ?, ?, ?, ?)").run(stableId(`${event}:${c.reel_id}:${c.calibration_version}`), "REEL", c.reel_id, event, actor, timestamp, JSON.stringify({ calibration_version: c.calibration_version, engine_version: c.engine_version, recommendation: c.recommendation, overall_score: c.overall_score }));
  if (c.suggested_package && Object.keys(c.suggested_package).length) db.prepare("INSERT OR IGNORE INTO publication_audit_events (event_id, entity_type, entity_id, event_type, actor, timestamp, metadata_json_safe) VALUES (?, ?, ?, ?, ?, ?, ?)").run(stableId(`editorial-suggestion:${c.reel_id}:${c.calibration_version}`), "REEL", c.reel_id, "AI_EDITORIAL_SUGGESTION_CREATED", actor, timestamp, JSON.stringify({ calibration_version: c.calibration_version, changed_fields: Object.keys(c.suggested_package) }));
}
