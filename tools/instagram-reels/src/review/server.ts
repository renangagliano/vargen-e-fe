...
async function handle(req: http.IncomingMessage, res: http.ServerResponse, config: MediaConfig, reviewPort: number): Promise<void> {
  ...
  if (reelMatch) {
    ...
    if (operation === "editorial") { const changes = { editorial_title: input.editorial_title, selected_hook: input.selected_hook, caption: input.caption, cta: input.cta, hashtags: input.hashtags, content_pillar: input.content_pillar, secondary_pillar: input.secondary_pillar ?? null, cover_text: input.cover_text, review_note: note || undefined }; const clean = Object.fromEntries(Object.entries(changes).filter(([, value]) => value !== undefined)) as never; const stored = await editReviewEditorial(reelId, actor, clean, config); const readBack = await getReviewItem(reelId, config); if (!readBack?.editorial || readBack.editorial.editorial_version !== stored.editorial_version) throw new Error("EDITORIAL_READ_AFTER_WRITE_FAILED"); json(res, 200, await withSession(readBack)); return; }
