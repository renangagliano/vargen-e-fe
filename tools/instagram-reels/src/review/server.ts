...
async function handle(req: http.IncomingMessage, res: http.ServerResponse, config: MediaConfig): Promise<void> {
  ...
  const reelMatch = url.pathname.match(/^\/api\/reels\/([^/]+)(?:\/(review|editorial|bible|readiness))?$/);
  if (reelMatch) {
