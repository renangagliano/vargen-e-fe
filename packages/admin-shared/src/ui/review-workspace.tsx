...
export function ReviewWorkspace({ initialData = EMPTY_DATA, readOnly = false, role = "VIEWER", candidateDetailEndpoint, mutationEndpoint }: { initialData?: ReviewWorkspaceData; readOnly?: boolean; role?: AdminRole; candidateDetailEndpoint?: string; mutationEndpoint?: string }) {
  ...
  return <main className="admin-workspace" aria-label="Review Workspace">
    ...
    {selected && <ReviewDrawer key={selected.reelId} row={selected} onClose={closeDrawer} onPersisted={() => router.refresh()} readOnly={readOnly} role={role} candidateDetailEndpoint={candidateDetailEndpoint} mutationEndpoint={mutationEndpoint} />}
  </main>;
}
...
function ReviewDrawer({ row, onClose, onPersisted, readOnly, role, candidateDetailEndpoint, mutationEndpoint }: { row: ReviewRow; onClose: () => void; onPersisted: () => void; readOnly: boolean; role: AdminRole; candidateDetailEndpoint?: string; mutationEndpoint?: string }) {
  ...
  const submitMutation = async (action: GovernanceMutationAction, extra: Record<string, unknown> = {}) => {
