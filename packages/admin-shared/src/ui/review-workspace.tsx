  ...
  return <div className="admin-drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <aside className="admin-drawer" role="dialog" aria-modal="true" aria-labelledby="review-drawer-title">
      ...
      <div className="admin-drawer__body">
        ...
        <section className="admin-drawer__section"><div className="admin-section-heading"><p className="admin-kicker">03 · Bíblia</p><StatusChip value={effectiveBibleStatus} /></div><div className="admin-readonly-fields">{editor("Referência", "bible_reference")}<p className="admin-muted">Evidência: {formatReviewStatus(valueOf(evidence, "evidence_status"))} · Verificação: {formatReviewStatus(effectiveBibleStatus)}</p>{verification && <p className="admin-muted">Validada por operador autenticado · {formatDate(valueOf(verification, "verified_at", ""))}</p>}</div></section>
        <section className="admin-drawer__section"><div className="admin-section-heading"><p className="admin-kicker">04 · Direitos</p><StatusChip value={effectiveRightsStatus} /></div><dl className="admin-detail-grid"><Detail label="Estado" value={formatReviewStatus(effectiveRightsStatus)} /><Detail label="Confirmação" value={latestRightsConfirmation ? "Confirmed" : "Não confirmados"} />{latestRightsConfirmation && <Detail label="Confirmado em" value={formatDate(valueOf(latestRightsConfirmation, "confirmed_at", ""))} />}</dl></section>
